...
def update_account_status_success(db_engine, account_id):
    """Mark report_status = 'delivered' for a successfully processed account."""
    try:
        with db_engine.begin() as conn:
            conn.execute(
                text("""
        ...
        logging.info(f"Account {account_id} marked as delivered.")
    except Exception as e:
        logging.error(f"Error marking account {account_id} as delivered: {e}")
        logging.error(traceback.format_exc())
        ...
        with ThreadPoolExecutor(max_workers=num_threads) as executor:
            ...
            for fut in as_completed(futures):
                ...
                try:
                    ...
                    # *** NEW: on full SUCCESS, mark as delivered ***
                    if email_type == "success":
                        update_account_status_success(db_engine, aid)
